...
export const documentTemplateService = {
  list: async (filters: DocumentTemplateListParams = {}, projectId?: number | null) => {
  ...
  getById: async (id: number, projectId?: number | null) => {
    const { data } = await api.get<DocumentTemplate>(`/data/document-templates/${id}/`, {
      params: params(projectId)
    })
    return data
  },

  create: async (payload: CreateDocumentTemplatePayload, projectId?: number | null) => {
    const { data } = await api.post<DocumentTemplate>('/data/document-templates/', payload, {
      params: params(projectId)
    })
    return data
  },

  update: async (id: number, payload: UpdateDocumentTemplatePayload, projectId?: number | null) => {
    const { data } = await api.patch<DocumentTemplate>(`/data/document-templates/${id}/`, payload, {
      params: params(projectId)
    })
    return data
  },
   ...
   */
  render: async (id: number, entityId: string, projectId?: number | null): Promise<Blob> => {
    const { data } = await api.post<Blob>(`/data/document-templates/${id}/render/`, null, {
