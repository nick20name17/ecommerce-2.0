...
export function ShippingRatesDialog({
  ...
  const validatePackages = (): boolean => {
    ...
    for (let i = 0; i < packages.length; i++) {
      ...
      if (pkg.length < MIN_DIMENSION)
        errors.push(`${label}: Length must be at least ${MIN_DIMENSION}.`)
      if (pkg.width < MIN_DIMENSION)
        errors.push(`${label}: Width must be at least ${MIN_DIMENSION}.`)
      if (pkg.height < MIN_DIMENSION)
        errors.push(`${label}: Height must be at least ${MIN_DIMENSION}.`)
    }
              ...
              <div className='flex items-center gap-0.5'>
                {(['configure', 'rates'] as const).map((s, i) => (
                  <span key={s} className='flex items-center gap-0.5'>
                    {i > 0 && <ChevronRight className='text-text-quaternary size-3' />}
                    <button
                      type='button'
                      className={cn(
            ...
            <>
              ...
              <div className='grid grid-cols-1 gap-3 border-b border-border px-4 py-4 sm:grid-cols-2 sm:px-5'>
                  ...
                  <Popover open={addressPopoverOpen} onOpenChange={setAddressPopoverOpen}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        ...
                        <ChevronDown className='text-text-quaternary size-3.5 shrink-0' />
                      </button>
                    </PopoverTrigger>
                    <PopoverContent
                      align='start'
                ...
                <div className='min-w-0'>
                  ...
                  >
                    <span className='min-w-0 flex-1 truncate text-[13px]'>
                      {hasAddress ? (
                        <>
                          ...
                          <span className='text-text-tertiary'>
                            {[address.c_address1, address.c_city, address.c_state, address.c_zip]
                              .filter(Boolean)
                              .join(', ')}
                          </span>
                        </>
                    ...
                    <Pencil className='text-text-quaternary size-3 shrink-0' />
                  </button>
                </div>
              </div>
                  ...
                  <div className='flex-1 space-y-2 overflow-y-auto p-3'>
                    {packages.length === 0 ? (
                      <div className='flex flex-col items-center justify-center py-12 text-center'>
                        <Package className='text-text-quaternary mb-2 size-6' />
                      ...
                      packages.map((pkg, i) => (
                        <PackageCard
                          ...
                          onUpdateDimension={(field, value) =>
                            updatePackageDimension(pkg.id, field, value)
                          }
                      ...
                      <tbody>
                        {items.map(item => {
                          ...
                          return (
                            ...
                            >
                              <td className='py-1.5 pr-0 pl-2'>
                                <GripVertical className='text-text-quaternary size-3.5' />
                              </td>
                              ...
                              <td className='px-3 py-1.5 text-right text-text-secondary tabular-nums'>
                                {parseFloat(String(item.quan))}
                              </td>
                              <td className='py-1.5 pr-3 pl-3 max-sm:hidden'>
                                <div className='flex items-center gap-1.5'>
                                  {item.is_picked ? (
              ...
              <div className='flex items-center justify-between border-t border-border px-4 py-3 sm:px-5'>
                <div className='text-[12px] text-text-tertiary max-sm:hidden'>
                  ...
                  {items.length} items assigned
                </div>
                <div className='text-[12px] text-text-tertiary sm:hidden'>
                  {packages.length} pkg · {items.length - unassignedItems.length}/{items.length}{' '}
                  assigned
                </div>
                <button
                  type='button'
