    ...
    <div className='flex h-full flex-col overflow-hidden'>
      ...
      >
        ...
        <div className='hidden h-7 w-full max-w-65 items-center gap-1.5 rounded-[5px] border border-border bg-background px-2 transition-[border-color,box-shadow] focus-within:border-ring focus-within:ring-2 focus-within:ring-ring/50 sm:flex'>
          ...
          <input
            aria-label='Search by email or EBMS ID'
            defaultValue={search}
      ...
      <div className='flex-1 overflow-y-auto'>
        {!isMobile && (carts.length > 0 || isLoading || isPlaceholderData) && (
          ...
          >
            <div className='w-17.5 shrink-0'>ID</div>
            <SortableHeader
              field='ebms_id'
              label='User'
              sortField={sortField}
            ...
            />
            <div className='w-27.5 shrink-0 text-right'>Total</div>
        ...
        {error ? (
          ...
          Array.from({ length: ABANDONED_CARTS_DEFAULT_LIMIT }).map((_, i) => (
            ...
            >
              ...
              <Skeleton className='h-3.5 w-50 rounded' />
