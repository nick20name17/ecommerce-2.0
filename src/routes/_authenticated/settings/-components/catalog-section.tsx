          ...
          <div className='flex items-start gap-3'>
            ...
            <div className='flex-1'>
              ...
              <p className='mt-0.5 text-[12px] text-text-tertiary'>
                ...
                EC_SPEC_DEFINITION, EC_SPEC_OPTION, and related tables in the mirror database. Safe
              ...
              <div className='mt-3 flex gap-2'>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (
                      confirm(
                        'This will DROP and recreate all EC_ tables. All catalog data will be lost. Continue?'
                      )
                    ) {
        ...
        <div className='rounded-lg border border-border bg-bg-secondary/40 p-4'>
          <div className='flex items-start gap-3'>
            ...
            <div className='flex-1'>
              ...
              <p className='mt-0.5 text-[12px] text-text-tertiary'>
                Runs Superinventory → Categories → Images in the correct dependency order in a
                single task. Use this for a fresh import or a normal re-import. Individual stages
                stay available under <em>Advanced</em> below for re-running a specific stage.
              </p>
              <div className='mt-3 flex flex-col gap-2'>
                ...
                {fullImportStatus?.status === 'running' && (
                  <div>
                    <div className='animate-pulse text-[12px] whitespace-pre-line text-text-tertiary'>
                ...
                {fullImportStatus?.status === 'completed' && (
                  <div className='rounded-md bg-bg-secondary p-2 text-[12px] whitespace-pre-line text-emerald-600 dark:text-emerald-400'>
                    Import complete
                    {fullImportStatus.result ? `: ${JSON.stringify(fullImportStatus.result)}` : ''}
                  </div>
                )}
        ...
        <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
          ...
          <CollapsibleContent className='mt-3 flex flex-col gap-4'>
            <p className='text-[12px] text-text-tertiary'>
              Re-run a single stage of the pipeline (e.g. only Images after fixing an asset).{' '}
              <strong>Order matters</strong> if running from scratch — Superinventory must run
              before Categories (Cats needs the VPs to link them), and Images runs last. Use{' '}
              <em>Import All</em> above for the normal flow.
            </p>
                ...
                <div className='flex-1'>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (importStatus.taskId) {
                            catalogImageService.cancelImport(importStatus.taskId, {
                              project_id: projectId
                            })
                            clearImportPolling()
                            clearTask('cat', projectId)
                  ...
                  {importStatus?.status === 'completed' && (
                    <div className='mt-2 rounded-md bg-bg-secondary p-2 text-[12px] text-emerald-600 dark:text-emerald-400'>
                      Import complete
                      {importStatus.result ? `: ${JSON.stringify(importStatus.result)}` : ''}
                    </div>
                  )}
                  ...
                  <div className='mt-3 flex flex-col gap-2'>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (vpImportStatus.taskId) {
                              catalogImageService.cancelImport(vpImportStatus.taskId, {
                                project_id: projectId
                              })
                              clearVPImportPolling()
                              clearTask('vp', projectId)
                    ...
                    {vpImportStatus?.status === 'completed' && (
                      <div className='rounded-md bg-bg-secondary p-2 text-[12px] text-emerald-600 dark:text-emerald-400'>
                        Import complete
                        {vpImportStatus.result ? `: ${JSON.stringify(vpImportStatus.result)}` : ''}
                      </div>
                    )}
                ...
                <div className='flex-1'>
                  ...
                  <div className='mt-3 flex flex-col gap-2'>
                    ...
                    {importSingleVPMutation.isSuccess && singleVPResult && (
                      <div className='rounded-md bg-bg-secondary p-2 text-[12px] text-emerald-600 dark:text-emerald-400'>
                        Imported:{' '}
                        {((singleVPResult as Record<string, unknown>).name as string) ??
                          singleSuperId}
                      </div>
                    )}
            ...
            <div className='rounded-lg border border-border p-4'>
              <div className='flex items-start gap-3'>
                ...
                <div className='flex-1'>
                  ...
                  <p className='mt-0.5 text-[12px] text-text-tertiary'>
                    Copies existing product and category images from the EBMS S3 bucket into our
                    catalog. Generates thumbnails automatically. Skips already imported images.
                  </p>
                  <Button
                    variant='outline'
                    size='sm'
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (imgImportStatus.taskId) {
                            catalogImageService.cancelImport(imgImportStatus.taskId, {
                              project_id: projectId
                            })
                            clearImgImportPolling()
                            clearTask('img', projectId)
                  ...
                  {imgImportStatus?.status === 'completed' && (
                    <div className='mt-2 rounded-md bg-bg-secondary p-2 text-[12px] text-emerald-600 dark:text-emerald-400'>
                      Import complete
                      {imgImportStatus.result ? `: ${JSON.stringify(imgImportStatus.result)}` : ''}
                    </div>
                  )}
