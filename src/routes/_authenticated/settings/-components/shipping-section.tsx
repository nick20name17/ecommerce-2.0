...
import {
  SHIPPING_ADDRESS_QUERY_KEYS,
  getShippingAddressesQuery
} from '@/api/shipping-address/query'
          ...
          <div className='max-w-xl space-y-4'>
            <div>
              ...
              <p className='mt-0.5 text-[13px] text-text-tertiary'>
                Enable shipping and configure ShipEngine for this project.
              </p>
            </div>
            ...
            <div className='flex items-center justify-between rounded-[8px] border border-border px-3.5 py-2.5'>
              <div>
                ...
                <p className='text-[12px] text-text-tertiary'>
                  Allow shipping rates, labels, and shipments
                </p>
              </div>
              <button
                type='button'
            ...
            {shippingEnabled && (
              <div className='rounded-[8px] border border-border px-3.5 py-2.5'>
                <div className='mb-1'>
                  ...
                  <p className='text-[12px] text-text-tertiary'>
                    Sandbox key for rate/label testing
                  </p>
                </div>
                <input
                  value={shipengineKey}
            ...
            {isLoading ? (
              <div>
                {Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className='flex items-center gap-6 border-b border-border-light px-6 py-1.5'
                  >
                  ...
                  <div className='hidden w-[160px] shrink-0 sm:block'>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        ...
                        <br />
                        {[addr.city, addr.state, addr.postal_code, addr.country_code]
                          .filter(Boolean)
                          .join(', ')}
                      </TooltipContent>
                    </Tooltip>
                  ...
                  <div className='flex w-[28px] shrink-0 items-center justify-center'>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button
                          type='button'
                          className='inline-flex size-6 items-center justify-center rounded-[5px] text-text-tertiary opacity-0 transition-all duration-75 group-hover/row:opacity-100 hover:bg-bg-active hover:text-foreground'
                        >
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Are you sure you want to delete &ldquo;{deleteAddress?.title}&rdquo;? This action
                  cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
