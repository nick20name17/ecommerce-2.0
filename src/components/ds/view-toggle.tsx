...
export function ViewToggle<T extends string>({
  ...
  return (
    <div className='flex items-center rounded-[5px] border border-border bg-background p-0.75'>
      {options.map(opt => (
        <button
          key={opt.value}
          type='button'
          className={cn(
            'inline-flex items-center gap-1 rounded-[3px] px-1.5 py-0.75 text-[13px] font-medium transition-colors duration-80',
            value === opt.value
